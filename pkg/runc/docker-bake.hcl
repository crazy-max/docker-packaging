...
# Sets the runc repo. Will be used to clone the repo at RUNC_REF ref to
# include the README.md and LICENSE for the static packages and also create
# version string.
variable "RUNC_REPO" {
...
target "_common" {
  ...
  cache-from = [
    BUILD_CACHE_REGISTRY_SLUG != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:runc-${PKG_TYPE}-${PKG_RELEASE}" : "",
  ]
  cache-to = [
    BUILD_CACHE_REGISTRY_SLUG != "" && BUILD_CACHE_REGISTRY_PUSH != "" ? "type=registry,ref=${BUILD_CACHE_REGISTRY_SLUG}:runc-${PKG_TYPE}-${PKG_RELEASE},mode=max" : "",
  ]
}
...
target "meta-helper" {
  tags = ["dockereng/packaging:runc-local"]
}
